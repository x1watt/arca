...
- **Packing** (`chain/packing.dart`).
  - A keeper stores each chunk XOR a keystream from `Argon2id(key, partition, index)`, expanded with SHA-256.
...
- **Clock and mining** (`chain/mining.dart`, milestone 3).
  ...
  - While no keeper has declared a partition, blocks need no proof, so a new chain can start.
...
- **Holding proofs** (`chain/state.dart`).
  - A day's beacon is the last block of the day before. From the beacon, the day and the keeper's key, each declared partition gets a slice to prove; the proof is the packed slice plus its Merkle paths, about 2 KB.
  - A keeper that has not proven every partition it declared before the day began, when the next day starts, loses all its declarations.
  - Holding proofs carry no nonce: replaying one changes nothing (it counts only on its own day), and a proof that missed its day, or was made on a fork that lost, must not hold back the keeper's later transactions. A keeper whose head moves to another fork makes the day's proof again against that fork's beacon.
- **Issuance and rewards** (`chain/rewards.dart`, milestone 4).
  - Blocks create no marcas. A day's issuance is paid as the day closes (in the first block of the next day), to the keepers who proved their keeping that day, into the pool of the circle each declaration names. What nobody earned is never created.
  ...
  - **Collections on the chain** come from the circles' anchors (below); genesis corpora carry a seed of interest. A keeper keeps a collection on a day when it proved all of its partitions.
  - **Interest budget, 70%:** a collection's interest is its seed, plus the marcas non-members burned for it over the last 30 days (`burn` transaction naming the collection; members are the admin, moderators and the circle's own keepers), plus a third of the standing of each keeper from another circle divided by the number of collections that keeper keeps. Each keeper's share of that interest follows the replication curve; a keeper's standing is the sum of its shares, and the budget is paid in proportion to it.
  - Standing is recomputed as each day closes from the standing of the day before, so it flows outward one hop per day. A keeper that misses a proof loses its standing with its declarations.
  - Checked against the whitepaper's claims (`test/chain_rewards_test.dart`): a circle keeping only its own archive earns storage and no interest; burns by members count for nothing; a ring of circles keeping each other's junk, besides the real data, reaches 1.125 times an honest keeper's standing with two circles and 1.286 times with five, under the 1.5 cap. Over live I2P (three keepers, one seeded collection per partition) every closed day paid exactly its issuance into the circle's pool, and each lone keeper's standing was a hundredth of its collection's seed, as the curve says.
- **Nodes** (`chain/node.dart`).
  ...
  - As a keeper, it mines each tick and posts its holding proof early each day by itself.
  - Tested in process (three keepers and one that declares a partition it does not keep: it is dropped on the next day, the others agree on one head and state root) on a loopback network that loses 30% of messages and delays them up to three ticks, and over live I2P (`tool/live_chain_check.dart`): three nodes on this desktop with 1 s ticks, 5-tick blocks and 60-tick days ran 64 blocks over five days, with the same head at every sample, all three keepers proving every day, and seven fork switches, all healed within a block or two. The first run, before the resync and the locator, failed: the keepers whose `declare` was lost never got in, and a node stayed on its own fork.
...
- **Circles** (`chain/circle_log.dart`, milestone 5).
  ...
  - A circle whose last anchor is more than a day old is cut off: its pool earns nothing and its collections earn no interest. Mining stays open to every keeper whatever its circle, so a chain where every circle lapsed can still include the anchors that bring them back. Genesis counts as the genesis circles' first anchor.
  - Payouts: the admin's software splits what the pool earned by the policy (45% keepers, 45% contributors, 10% moderators by default; `distribute`) into cumulative totals per member, written to the log as a payout entry that can only grow. The anchor commits them as a Merkle root; a member's `claim` shows its leaf and receives its total minus what it claimed before. A claim above the table, someone else's line, or more than the pool holds fails.
...
- **Fraud proofs** (`chain/fraud.dart`, milestone 6).
  ...
  - Sizes: a transfer's proof is about 5 KB. A day's settlement goes through every keeper, so its proof carries those namespaces whole: about 1.5 KB per keeper, 600 KB with 400 keepers, sent in parts. That is fine for the testnet and too much for phones at mainnet size; settling a day in many small steps, each provable alone, is an open problem.
