...
Future<void> main(List<String> args) async {
  ...
  say('corpus: ${corpus.chunks.length} chunks, ${corpus.partitions} partitions, root ${corpus.rootHex.substring(0, 16)}');
  if (corpus.partitions < 3) exit(1);

  final cores = <CoreService>[];
  for (final n in ['a', 'b', 'c']) {
    cores.add(await CoreService.open('$dir/$n', startNetwork: false, backend: I2pBackend('$dir/$n/i2p'), defaultBaseFolder: '$dir/$n/Arca'));
  }
  ...
  for (var i = 0; i < 3; i++) {
    final steward = Steward(params: live, key: toHex(publicKeyOf(keys[i])), corpus: corpus, folder: '$dir/packed-$i', files: files);
  ...
  for (var i = 0; i < 3; i++) {
    nodes[i].submit(TxType.declare, {'circle': 'commons', 'partitions': [i]});
  }
  ...
  say('height ${s.height}, day ${s.day}, pool ${(s.circles['commons']!.pool / ChainParams.grainsPerMarca).toStringAsFixed(0)} marcas, '
      'issued ${(s.issued / ChainParams.grainsPerMarca).toStringAsFixed(0)}');
  for (var i = 0; i < 3; i++) {
    say('steward ${'abc'[i]} declared: ${s.declarations[nodes[i].key] ?? 'dropped'}, last proof on day ${s.provenOn[nodes[i].key]?[i]}');
  }
  final ok = heads.length == 1 && roots.length == 1 && s.declarations.length == 3 && s.day >= 2;
