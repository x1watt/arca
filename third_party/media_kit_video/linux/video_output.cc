...
struct _VideoOutput {
  ...
  gboolean gl_failed; /* Arca: set when the lazy EGL setup failed. */
...
static void video_output_dispose(GObject* object) {
  ...
  if (self->texture_gl) {
    ...
    if (self->render_context != NULL) {
      ...
      // Restore Flutter's context, or leave none current on this thread.
...
VideoOutput* video_output_new(FlTextureRegistrar* texture_registrar,
  ...
  if (self->configuration.enable_hardware_acceleration) {
    // Arca: Flutter renders with its own EGL context on the raster thread;
    // this (platform) thread holds GDK's GLX context, so there is no EGL
    // context to copy here. The mpv render context is created on the
    // raster thread the first time Flutter asks for a frame
    // (video_output_ensure_gl, called from texture_gl_populate_texture).
    self->texture_gl = texture_gl_new(self);
    if (fl_texture_registrar_register_texture(texture_registrar,
                                              FL_TEXTURE(self->texture_gl))) {
      hardware_acceleration_supported = TRUE;
      // Flutter only asks for a frame once the texture is on screen and
      // marked available; keep marking it until the GL setup has run.
      g_timeout_add(
          30,
          [](gpointer data) -> gboolean {
            VideoOutput* self = (VideoOutput*)data;
            if (self->destroyed || self->gl_failed || self->render_context != NULL) {
              g_object_unref(self);
...
gboolean video_output_ensure_gl(VideoOutput* self) {
  if (self->render_context != NULL) {
    return TRUE;
  }
  if (self->gl_failed || self->destroyed) {
    return FALSE;
  }
  self->gl_failed = TRUE;  // Cleared below once everything is in place.

  // Called on Flutter's raster thread, where its EGL context is current.
  EGLDisplay flutter_display = eglGetCurrentDisplay();
  EGLContext flutter_context = eglGetCurrentContext();
  EGLSurface flutter_draw_surface = eglGetCurrentSurface(EGL_DRAW);
  EGLSurface flutter_read_surface = eglGetCurrentSurface(EGL_READ);
  if (flutter_display == EGL_NO_DISPLAY || flutter_context == EGL_NO_CONTEXT) {
    g_printerr("media_kit: VideoOutput: No EGL context on the raster thread.\n");
    return FALSE;
  }
  self->egl_display = flutter_display;
  eglBindAPI(EGL_OPENGL_ES_API);

  // Same config as Flutter's, so the EGLImage can be used by both.
  EGLConfig config = NULL;
  EGLint config_id = 0, num_configs = 0;
  ...
  if (!eglChooseConfig(flutter_display, config_attribs, &config, 1, &num_configs) ||
      num_configs < 1) {
    g_printerr("media_kit: VideoOutput: Failed to get Flutter's EGL config.\n");
    return FALSE;
  }
  // GLES 3 when the driver has it: mpv's hardware decoding interops
  // (CUDA, VAAPI) need more than GLES 2.
  EGLint es3_attribs[] = {EGL_CONTEXT_CLIENT_VERSION, 3, EGL_NONE};
  EGLint es2_attribs[] = {EGL_CONTEXT_CLIENT_VERSION, 2, EGL_NONE};
  self->egl_context = eglCreateContext(flutter_display, config, EGL_NO_CONTEXT, es3_attribs);
  if (self->egl_context == EGL_NO_CONTEXT) {
    self->egl_context = eglCreateContext(flutter_display, config, EGL_NO_CONTEXT, es2_attribs);
  }
  if (self->egl_context == EGL_NO_CONTEXT) {
    g_printerr("media_kit: VideoOutput: Failed to create EGL context: 0x%x\n", eglGetError());
    return FALSE;
  }
  if (!eglMakeCurrent(flutter_display, EGL_NO_SURFACE, EGL_NO_SURFACE, self->egl_context)) {
    g_printerr("media_kit: VideoOutput: Failed to make EGL context current: 0x%x\n", eglGetError());
    return FALSE;
  }
  ...
  // Hardware decoding (VAAPI) needs the window system's display.
  ...
  gboolean ok = mpv_render_context_create(&self->render_context, self->handle, params) == 0;
  if (ok) {
    mpv_render_context_set_update_callback(
        self->render_context,
        [](void* data) {
    ...
    // during playback, so name the track. Asynchronous: this is the raster
    // thread.
    char* vo = mpv_get_property_string(self->handle, "current-vo");
    char* path = mpv_get_property_string(self->handle, "path");
    if ((vo == NULL || vo[0] == '\0') && path != NULL) {
      const char* select[] = {"set", "vid", "1", NULL};
      mpv_command_async(self->handle, 0, select);
    }
    if (vo) mpv_free(vo);
